...
label0:
        {
            int i = 0;
            for(int j = SYMBOL_TABLE.length; i < j;)
            {
                k = i + j >>> 1;
    ...
    {
        int i = -1;
        int j = findCodewordIndex(l & 0x3ffffL);
        if(j != i)
            i = (int)((long)(-1 + CODEWORD_TABLE[j]) % 929L);
        return i;
    }
...
_L3:
        int l2;
        int i3 = getCodeword(l1);
        if(i3 < 0 && j1 < l - 17)
        {
            erasures[eraseCount] = k1;
            l2 = k1 + 1;
...
_L1:
        if(l < 90) goto _L4; else goto _L3
_L3:
        int ai2[] = null;
_L8:
        return ai2;
_L4:
        ...
        for(int l1 = 0; l1 < i; l1++)
            if(bitMatrix.get(l1, j1) != bitMatrix.get(l1, j1 - 1))
                k1++;

        if((float)k1 > 6F * 1.0F) goto _L6; else goto _L5
_L5:
        for(int j2 = 0; j2 < i; j2++)
            if(bitMatrix.get(j2, j1))
                ai[j2] = 1 + ai[j2];

        if((float)(++k) >= 2.0F * 1.0F)
            flag = true;
_L9:
        i1++;
        j1++;
