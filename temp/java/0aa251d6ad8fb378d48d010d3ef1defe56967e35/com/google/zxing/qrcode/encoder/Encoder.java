...
_L2:
        ...
        throws WriterException
    {
        ...
        if(i > -1 + (1 << k))
        {
            throw new WriterException(i + "is bigger than" + (-1 + (1 << k)));
        } else
        {
    ...
    static void appendNumericBytes(String s, BitArray bitarray)
    {
        ...
        for(int j = 0; j < i;)
        {
            ...
            if(j + 2 < i)
            {
                int l = -48 + s.charAt(j + 1);
                bitarray.appendBits(-48 + s.charAt(j + 2) + (k * 100 + l * 10), 10);
                j += 3;
            } else
            if(j + 1 < i)
            {
                bitarray.appendBits(-48 + s.charAt(j + 1) + k * 10, 7);
                j += 2;
            } else
            {
...
_L3:
        ...
        throws WriterException
    {
        ...
        if(mode == Mode.BYTE && !"UTF-8".equals(s1))
        {
        ...
        throws WriterException
    {
        ...
        for(int k1 = 0; k1 < k; k1++)
        {
            int ai[] = new int[1];
            int ai1[] = new int[1];
            getNumDataBytesAndNumECBytesForBlockID(i, j, k, k1, ai, ai1);
            int l2 = ai[0];
            byte abyte2[] = new byte[l2];
            bitarray.toBytes(l * 8, abyte2, 0, l2);
            byte abyte3[] = generateECBytes(abyte2, ai1[0]);
            vector.addElement(new BlockPair(abyte2, abyte3));
            i1 = Math.max(i1, l2);
            j1 = Math.max(j1, abyte3.length);
            l += ai[0];
        }
