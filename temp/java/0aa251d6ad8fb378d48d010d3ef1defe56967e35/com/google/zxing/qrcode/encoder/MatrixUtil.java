...
public final class MatrixUtil
{
        ...
        throws WriterException
    {
        int j = 0;
        int k = -1;
        int l = -1 + bytematrix.getWidth();
        int i1 = -1 + bytematrix.getHeight();
        for(; l > 0; l -= 2)
        {
            if(l == 6)
                l--;
            for(; i1 >= 0 && i1 < bytematrix.getHeight(); i1 += k)
            {
                int j1 = 0;
                while(j1 < 2) 
                {
        ...
        if(j != bitarray.getSize())
            throw new WriterException("Not all bits consumed: " + j + '/' + bitarray.getSize());
        else
            return;
    }
        ...
        throws WriterException
    {
        ...
        embedVerticalSeparationPattern(-1 + (bytematrix.getHeight() - k), 0, bytematrix);
    ...
    private static void embedTimingPatterns(ByteMatrix bytematrix)
        throws WriterException
    {
        for(int i = 8; i < -8 + bytematrix.getWidth(); i++)
        {
            int j = (i + 1) % 2;
        ...
        throws WriterException
    {
        ...
        {
            boolean flag = bitarray.get((-1 + bitarray.getSize()) - j);
            bytematrix.set(TYPE_INFO_COORDINATES[j][0], TYPE_INFO_COORDINATES[j][1], flag);
            if(j < 8)
                bytematrix.set(-1 + (bytematrix.getWidth() - j), 8, flag);
            else
                bytematrix.set(8, -7 + bytematrix.getHeight() + (j - 8), flag);
            j++;
        }
    }
        ...
        if(i >= 7)
        {
            ...
            while(k < 6) 
            {
                for(int l = 0; l < 3; l++)
                {
                    ...
                    j--;
                    bytematrix.set(k, l + (-11 + bytematrix.getHeight()), flag);
                    bytematrix.set(l + (-11 + bytematrix.getHeight()), k, flag);
                }
