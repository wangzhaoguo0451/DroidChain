...
class Permission_Sort (QDialog,permission_sort):
	...
	def on_pushButton_ok_clicked(self, checked):
		...
		if self.checkBox_cfg.isChecked():
			...
			self.pipei_apk_BG_temp(_fromUtf8("硬件层"))
		else:
...
# 							print jjj
					...
					for m1 in range(int(count)+1):
						...
						#  		G1=zeros((int(count)+1,int(count)+1))			print "->"
									write_usedpath_content +="->"+"\n"
									print m2
	...
	def pipei_apk_BG(self,function_name):
		...
		for name in Master_filename:
			if not zipfile.is_zipfile(name):
				...
				Writename=name[:-4]+time_filename+"_report_For_Detail.txt"
				Writename_user=name[:-4]+time_filename+"_report_For_user.txt"				
...
# 							print key_s
					  ...
					  print pathb
 					  try:
 					  	...
 					  	for key in key_d.keys():
 					  		...
 					  		report +="方式泄漏.\n\n"
 					  		report_user +="   该软件涉嫌"
 					  		report_user +=json.dumps(yinsiqiequ[xinxi_qiequ_yuan[des1][3]],encoding="UTF-8",ensure_ascii=False)
 					  		report_user +="隐私信息,并通过"
 					  		report_user +=json.dumps(yinsiqiequ[xinxi_xie_loudian[des2][3]],encoding="UTF-8",ensure_ascii=False)
 					  		report_user +="方式泄漏.\n\n"
 					  		for key_des in xinxi_qiequ_yuan.keys():
 					  			if key.find(key_des)!=-1:
 					  	...
 					  	os.system("python /home/wang/Desktop/v1.5/startQT.py")
 					  	continue
		...
		text_report.write("\n")
		text_report.close()
		text_report_user=open(Writename_user,'a+')
		text_report_user.write(report_user)
		text_report_user.write("\n")
		text_report_user.close()
		report =""
		report_user =""
	def pipei_apk_BG_temp(self,function_name):
		global SYSTEMLINE
		if Global.Permission_CONFIG["1"]==1:
			dlg = QFileDialog(self)  # @UndefinedVariable
			zifilename = dlg.getOpenFileName(self, _fromUtf8("请打开")+function_name+_fromUtf8("敏感权限样本文件"), QString(),  _fromUtf8(" *.* 文件(*.*)"))
			texts=open(zifilename,'r+')
			text_s=texts.readlines()
		dlgs = QFileDialog(self)  # @UndefinedVariable
		Master_filename = dlgs.getOpenFileNames(self, _fromUtf8("请打开（可以选择多个文件）APK文件"), QString(),  _fromUtf8("APK 文件(*.apk)"))
